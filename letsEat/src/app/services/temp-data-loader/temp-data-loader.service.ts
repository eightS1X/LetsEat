...
//  for uploading / providing an input file to the application
  ...
  async loadFileContentsIntoCookie(inputAssetFilePath: string, cookieName: string) {
    this.http.get(inputAssetFilePath, { responseType: 'text' })
      .subscribe(
        (data: string) => {
          ...
          if (!this.cookieService.checkCookieExists(cookieName)) {
            this.cookieService.saveDataToCookie(cookieName, delimitedString);
          };
        },
        (error) => {
